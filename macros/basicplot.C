void basicplot(int En=5){
  ...
  TCanvas *c2 = new TCanvas("c2","c2", 100, 100, 2000,2000);
  c1->Divide(3,2);
  c2->Divide(3,2);
  ...
  for(int jj=0;jj<Np;jj++){
   for(int ii=0;ii<Ndet;ii++){
     if(ii==0){
     c1->cd(1+(jj)*3); // 00, 01,10,11 1, 3, 5, 7
     }
     if(ii==1){
     c2->cd(1+(jj)*3); // 00, 01,10,11 1, 3, 5, 7
     }
  ...
  cout<<cx<<" ";
    ...
    //     cout<<N<<endl;
     ...
     if(ii==1){
       c2->cd(3+(jj)*3);
        ...
        t->Draw(Form("%s.e>>hE%d_%d",Dets[ii].Data(),ii,jj),Form("%s.pID==%d",Dets[ii].Data(),pID[jj]));
	...
	if(ii==0){
     ...
     c1->cd(3+(jj)*3)->SetLogy(1);
	}
	if(ii==1){
     c2->cd(1+(jj)*3)->SetLogy(1);
     c2->cd(2+(jj)*3)->SetLogy(1);
     c2->cd(3+(jj)*3)->SetLogy(1);
	}
